...
EC_POINT *multiplyConstInt(const EC_POINT *in, int n) {
	BIGNUM *bnI;
	EC_POINT *res;
	bnI = BN_new();
	BN_zero(bnI);
...
void Java_ch_ethz_inf_vs_lubu_cyrptdbmodule_cryptoalg_FastECElGamal_initTable(JNIEnv* env,
		jobject javaThis) {
	if (table != NULL) {
...
void Java_ch_ethz_inf_vs_lubu_cyrptdbmodule_cryptoalg_FastECElGamal_destroyTable(JNIEnv* env,
		jobject javaThis) {
	int k;
	if (table != NULL) {
...
void Java_ch_ethz_inf_vs_lubu_cyrptdbmodule_cryptoalg_FastECElGamal_setTableSizeConstants(
		JNIEnv* env, jobject javaThis, jint size) {
	setTableSizeConstants((int) size);
}

void Java_ch_ethz_inf_vs_lubu_cyrptdbmodule_cryptoalg_FastECElGamal_computeTable(JNIEnv* env,
...
jint Java_ch_ethz_inf_vs_lubu_cyrptdbmodule_cryptoalg_FastECElGamal_putInTable(JNIEnv* env,
...
jint Java_ch_ethz_inf_vs_lubu_cyrptdbmodule_cryptoalg_FastECElGamal_getFromTable(JNIEnv* env,
...
int computePower(int i, int j) {
	return i * tableSize + j;
}

int solveECDLPBsGs(EC_POINT *M, int maxIt) {
	int j = 0, i = 0, iNeg = 0, res;
	EC_POINT *curPoint = EC_POINT_dup(M, curveGroup);
	EC_POINT *curPointNeg = EC_POINT_dup(M, curveGroup);
	BN_CTX *ctx = BN_CTX_new();
	int found = 0;
	while (i <= maxIt) {
		j = getPowerFromTable(curPoint);
		if (j != -1) {
		...
		j = getPowerFromTable(curPointNeg);
		if (j != -1) {
			res = computePower(iNeg, j);
			break;
		}
 ...
 * Solves ECDLP pos
 */
jint Java_ch_ethz_inf_vs_lubu_cyrptdbmodule_cryptoalg_FastECElGamal_solveECDLPBsGsPos(JNIEnv* env,
		jobject javaThis, jobject state, jstring M, jint startI, jint endI) {
	const char *Mstr;
	EC_POINT *Mpoint, *temp;
	BN_CTX *ctx = BN_CTX_new();
	int i,j, maxi, res, found = 1, MUTEX;
	...
	temp = multiplyConstInt(mGInv,(int) startI);
...
jint Java_ch_ethz_inf_vs_lubu_cyrptdbmodule_cryptoalg_FastECElGamal_solveECDLPBsGsNeg(JNIEnv* env,
		jobject javaThis, jobject state, jstring M, jint startI, jint endI) {
	const char *Mstr;
	EC_POINT *Mpoint, *temp;
	BN_CTX *ctx = BN_CTX_new();
	...
	(*env)->ReleaseStringUTFChars(env, M, Mstr);
	EC_POINT *curPoint = EC_POINT_new(curveGroup);
	temp = multiplyConstInt(mG,(int) startI);
	EC_POINT_add(curveGroup, curPoint, Mpoint, temp, ctx);
	EC_POINT_free(temp);
 ...
 */
jstring Java_ch_ethz_inf_vs_lubu_cyrptdbmodule_cryptoalg_FastECElGamal_encryptNative(JNIEnv* env,
 ...
 * Solves ECDLP
 */
jint Java_ch_ethz_inf_vs_lubu_cyrptdbmodule_cryptoalg_FastECElGamal_solveECDLPBsGs(JNIEnv* env,
		jobject javaThis, jstring M, jint maxIt) {
	const char *Mstr;
	EC_POINT *Mpoint;
	Mstr = (*env)->GetStringUTFChars(env, M, 0);
	Mpoint = convertToPoint(Mstr);
	int res = solveECDLPBsGs(Mpoint, (int) maxIt);
	(*env)->ReleaseStringUTFChars(env, M, Mstr);
	EC_POINT_free(Mpoint);
	return (jint) res;
}
 ...
 */
jstring Java_ch_ethz_inf_vs_lubu_cyrptdbmodule_cryptoalg_FastECElGamal_decryptNative(JNIEnv* env,
...
jstring Java_ch_ethz_inf_vs_lubu_cyrptdbmodule_cryptoalg_FastECElGamal_addCipherNative(JNIEnv* env,
...
jstring Java_ch_ethz_inf_vs_lubu_cyrptdbmodule_cryptoalg_FastECElGamal_computePubKey(JNIEnv* env,
 ...
 */
jstring Java_ch_ethz_inf_vs_lubu_cyrptdbmodule_cryptoalg_FastECElGamal_getCurveOrder(JNIEnv* env,
		jobject javaThis) {
	BIGNUM *order;
	jstring res;
	BN_CTX *ctx;
 ...
 */
jstring Java_ch_ethz_inf_vs_lubu_cyrptdbmodule_cryptoalg_FastECElGamal_getPrimeOfGf(JNIEnv* env,
		jobject javaThis) {
	BIGNUM *prime;
	jstring res;
 ...
 * Free static curve
 */
void Java_ch_ethz_inf_vs_lubu_cyrptdbmodule_cryptoalg_FastECElGamal_tearDOWN(JNIEnv* env,
		jobject javaThis) {
	if (curveGroup != NULL) {
 ...
 * 0 -> default NIST-p192
 */
void Java_ch_ethz_inf_vs_lubu_cyrptdbmodule_cryptoalg_FastECElGamal_setUP(JNIEnv* env,
 ...
 */
jstring Java_ch_ethz_inf_vs_lubu_cyrptdbmodule_cryptoalg_FastECElGamal_computeGenTimes(JNIEnv* env,
